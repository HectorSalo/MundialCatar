...
import com.skysam.hchirinos.mundial2026.databinding.FragmentPredictsBinding
...
class PredictsFragment : Fragment() {
    ...
    private fun loadViewModel() {
        ...
        viewModel.games.observe(viewLifecycleOwner) {
            if (_binding == null) return@observe
            games = it.filter { game ->
                game.stage == MatchStage.GROUP ||
                        (game.canPredicted != null && game.canPredicted)
            }
            joinData()
        }
    ...
    private fun fillData(gamesToView: List<GameToView>, canEditMap: Map<Int, Boolean>) {
        ...
        val endOfToday = Calendar.getInstance().apply {
            time = startOfToday.time
            add(Calendar.DAY_OF_MONTH, 1)
        }

        val todayIndex = gamesToView.indexOfFirst { game ->
            !game.date.before(startOfToday.time) && game.date.before(endOfToday.time)
        }

        val fallbackIndex = gamesToView.indexOfFirst { game ->
            !game.date.before(startOfToday.time)
        }

        val targetIndex = when {
            todayIndex != -1 -> todayIndex
            fallbackIndex != -1 -> fallbackIndex
            else -> -1
        }

        if (moveList && targetIndex != -1) {
            binding.rvGames.post {
                val layoutManager =
                    binding.rvGames.layoutManager as? androidx.recyclerview.widget.LinearLayoutManager

                layoutManager?.scrollToPositionWithOffset(targetIndex, 0)
                    ?: binding.rvGames.scrollToPosition(targetIndex)
            }
            moveList = false
        }
    }
